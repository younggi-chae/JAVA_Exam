...
public class MemberDao {
	...
	public int findByUsernameAndPassword(String username, String password) {
		...
		try {
			// 2. Query 작성
			pstmt = conn.prepareStatement("select * from management where username = ? and password = ?");
	...
	public int save(Member member) {
		...
		try {
			pstmt = conn.prepareStatement("insert into management(id,username,password,realname,email,phone,createdate) "
					+ "values((select ifnull(max(id),0)+1 from management as a),?,?,?,?,?,NOW())");
	...
	public Vector<Member> findByAll() {
		...
		try {
			pstmt = conn.prepareStatement("select * from management");
			rs = pstmt.executeQuery();
			while (rs.next()) {
				...
				member.setName(rs.getString("realname"));
